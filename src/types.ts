...
// Wire-level payment types are protocol surface, so core owns them and hands
// them on; `./payments` is a consumer of this file, not a dependency of it.
export type * from './payment-wire'
...
export interface SoulPassWalletConfig {
  ...
  walletUrl?: string
  /**
   * Override sign-channel relay API base (default: derived from walletUrl —
   * `soulpass.ai` → `https://api.soulpass.ai/api`, env-prefixed hosts to
   * their env api hosts). Only the dual-channel sign flow uses this.
   */
  apiUrl?: string
  /**
   ...
   *
   * **First-party Matrix products: set this** so sessions land in your
   * canonical namespace (the one your Gateway domain reads and your revoke
   * flows target).
   *
   * **Third-party dApps: omit it.** The popup forwards your (postMessage-
   * verified) origin instead, and the backend derives an isolated
   * `ext-{host}` session namespace from it — zero registration, no
   * collision with first-party sessions. Claiming a first-party name buys
   * nothing: session forwarding is gated on origin, not on this string.
   */
  productType?: string
}
...
/** Derived from the {@link SDKMessage} union — a new message variant can't
 * land there and silently miss this type. */
export type SDKMessageType = SDKMessage['type']
...
/**
 * First leg of one /wallet/pay session: render the intent and discover balances.
 *
 ...
 * renders only that — the relayed `intent` is a hint for instant first paint,
 * never the source of truth, because everything in this payload arrives from
 * the merchant page. The wallet hard-fails without a token, so it is required.
 */
export interface SDKPaymentDiscoverMessage {
  type: 'PAYMENT_DISCOVER'
  id: string
  payload: { intent: PaymentIntent; displayToken: string }
}

/**
 * Between the two legs the merchant calls the payment API to lock an attempt,
 * which the popup cannot observe — without this notice it renders a frozen
 * balance list for the whole server round-trip. Advisory: the popup may ignore
 * it, and the SDK never waits for a reply.
 */
export interface SDKPaymentPreparingMessage {
  type: 'PAYMENT_PREPARING'
  id: string
  payload: { settlementOptionId: string }
}

/** Second leg: authorize and submit the server-prepared chain execution. */
export interface SDKPaymentExecuteMessage {
  type: 'PAYMENT_EXECUTE'
  id: string
  payload: { execution: PaymentExecution }
}
...
export interface PopupPaymentAccountsMessage {
  type: 'PAYMENT_ACCOUNTS'
  id: string
  payload: { accounts: readonly PaymentAccount[] }
}

export interface PopupPaymentSuccessMessage {
