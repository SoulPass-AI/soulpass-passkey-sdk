...
import { base58ToPubkeyBytes } from './base58';
 ...
 */
/**
 * Compose an account's flag byte. Shared by the hash below and the ix
 * encoding in `execute-ix.ts` — the two MUST agree bit-for-bit, or the
 * chain's re-hash fails with `MessageMismatch` at submit time.
 */
export function accountFlags(acc: InnerInstruction['accounts'][number]): number {
  let flags = 0;
...
export function computeInnerHash(
  ...
  for (const ix of innerInstructions) {
    const programIdBytes = base58ToPubkeyBytes(ix.programId);
    const parts: Uint8Array[] = [programIdBytes, u16LE(ix.accounts.length)];
    for (const acc of ix.accounts) {
      parts.push(base58ToPubkeyBytes(acc.pubkey));
      parts.push(Uint8Array.of(accountFlags(acc)));
    }
