 ...
 * message tail. Empty or oversized `clientDataJSON` is rejected by the
 * program (`MAX_CLIENT_DATA_JSON_SIZE`).
 */
export function buildEvidenceIxData(clientDataJSON: Uint8Array): Uint8Array {
  ...
  if (clientDataJSON.length > MAX_CLIENT_DATA_JSON_SIZE) {
    throw new RangeError(
      `clientDataJSON exceeds on-chain sidecar limit: ` +
        `${clientDataJSON.length} > ${MAX_CLIENT_DATA_JSON_SIZE} bytes`,
    );
  }
  return concatBytes([
...
export function buildExecuteIxData(args: {
  ...
  for (const ix of innerInstructions) {
    ...
    for (let i = 0; i < ix.accounts.length; i++) {
      ...
      // Shared with computeInnerHash — the hashed flags and the encoded flags
      // must agree or the chain re-hash fails with MessageMismatch.
      accountEntries[i * 2 + 1] = accountFlags(acc);
    }
