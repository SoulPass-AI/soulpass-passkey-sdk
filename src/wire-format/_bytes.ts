...
/** Assert a fixed-width payload field, returning it for inline use. */
export function requireLength(bytes: Uint8Array, expected: number, label: string): Uint8Array {
  if (bytes.length !== expected) {
    throw new RangeError(`${label} must be ${expected} bytes, got ${bytes.length}`);
  }
  return bytes;
}

/** Validate and encode a single u8 payload field. */
export function requireByte(value: number, label: string): Uint8Array {
  if (!Number.isInteger(value) || value < 0 || value > 0xff) {
    throw new RangeError(`${label} must be a u8, got ${value}`);
  }
